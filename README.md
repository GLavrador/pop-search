...
1. **Authentication → Providers → Email**: habilite. Desmarcar *Confirm email*
   facilita o desenvolvimento; marque antes de publicar.
2. **Authentication → URL Configuration**: `Site URL` = `http://localhost:5173`
3. **Settings → API**: copie a chave `anon` e a `service_role`
4. **Settings → API → JWT Secret**: copie o segredo
...
VITE_SUPABASE_ANON_KEY="..."   # anon, pública por natureza
```
